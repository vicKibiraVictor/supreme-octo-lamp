...
use revm::{
    ...
    primitives::{
        keccak256 as revm_keccak256, AccountInfo, Address as B160, ExecutionResult,
...
impl Pool {
    ...
    fn hook_accepted(&self) -> bool { self.hook == H160::zero() }
    fn liquidity_ok(&self) -> bool  { self.liquidity >= MIN_LIQUIDITY_RAW }
}
...
impl EngineState {
    ...
    fn upsert_pool_edges(&mut self, pool: &Pool) {
        ...
        if !pool.liquidity_ok() {
            debug!(pool_id = hex::encode(pool.pool_id), liq = pool.liquidity,
        ...
        if r10 > 0.0 {
            self.meta_graph.add_edge(n0, n1, Edge {
                pool_id: pool.pool_id, zero_for_one: true,
                weight: -r10.ln(), rate: r10,
            });
        }
        if r01 > 0.0 {
            self.meta_graph.add_edge(n1, n0, Edge {
                pool_id: pool.pool_id, zero_for_one: false,
                weight: -r01.ln(), rate: r01,
            });
        }
...
fn parse_initialize_log(log: &ethers::types::Log) -> Option<Pool> {
    ...
    Some(Pool {
        ...
        lp_fee: fee, protocol_fee: 0,
        hook, tick_spacing, liquidity: 0,
        sqrt_price_x96: sqrt_price, tick,
    })
}
...
async fn discover_pools(provider: &Provider<Http>) -> Result<Vec<Pool>> {
    ...
    while from <= latest {
        ...
        for log in &logs {
            if let Some(pool) = parse_initialize_log(log) { pools.push(pool); }
        }
...
fn find_triangular_cycles(state: &EngineState) -> Vec<[NodeIndex; 3]> {
    ...
    for start in state.weight_graph.node_indices() {
        ...
        if let Some(cycle) = find_negative_cycle(&state.weight_graph, start) {
            ...
            key.sort_unstable();
            if seen.insert(key) { out.push([cycle[0], cycle[1], cycle[2]]); }
        }
    }
    out
}
...
fn build_candidate(
    ...
    for i in 0..3 {
        ...
        keys.push(PoolKey {
            ...
            fee: pool.lp_fee, tick_spacing: pool.tick_spacing, hook: pool.hook,
        });
    }
...
async fn find_balance_slot(provider: &Provider<Http>, token: H160) -> u32 {
    ...
    for slot in 0u32..30 {
        ...
        if stored_u256 == real_bal && !real_bal.is_zero() { return slot; }
    }
...
async fn simulate(
    ...
    cache_db
        .insert_account_storage(token_a_b160, vault_balance_slot, rU256::from(c.loan_amount.as_u128()))
    ...
    let result = evm.transact_commit().map_err(|e| anyhow!("REVM transact_commit: {:?}", e))?;
    ...
    if gross_gain <= gas_cost {
        debug!(gross = ?gross_gain, gas = ?gas_cost, "sim: profitable before gas but not after");
        return Ok((false, U256::zero()));
    }
...
async fn submit_bundle(
    ...
    let tx: TypedTransaction = Eip1559TransactionRequest::new()
        .to(executor_addr).data(calldata).nonce(nonce).gas(GAS_LIMIT)
        .max_priority_fee_per_gas(priority_fee).max_fee_per_gas(max_fee)
        .chain_id(chain_id)
        .into();
    ...
    let pending = fb_middleware.send_bundle(&bundle).await.context("send_bundle failed")?;
...
impl Controller {
    fn new(
        http_rpc: String, ws_rpc: String, private_key: &str,
        executor_addr: H160, treasury: H160,
    ) -> Result<Self> {
        let wallet = private_key
            .parse::<LocalWallet>().context("invalid PRIVATE_KEY")?
            .with_chain_id(CHAIN_ID);
        Ok(Self { http_rpc, ws_rpc, wallet, executor_addr, treasury,
    ...
    async fn run(&self) -> Result<()> {
        ...
        info!(pools = self.state.read().await.pools.len(), "Bootstrap complete");
    ...
    async fn event_loop(&self) -> Result<()> {
        let ws = Ws::connect(&self.ws_rpc).await.context("WebSocket connect failed")?;
    ...
    async fn refresh_pool(&self, provider: &Provider<Http>, pool_id: PoolId) {
        ...
        match enrich_pool(provider, pool).await {
            Ok(()) => { let updated = pool.clone(); state.upsert_pool_edges(&updated); }
            Err(e) => {
    ...
    async fn search_and_execute(&self, provider: &Provider<Http>) -> Result<()> {
        ...
        let mut candidates: Vec<Candidate> = {
            let state = self.state.read().await;
            cycles.iter().filter_map(|cyc| build_candidate(cyc, &state, gas_price)).collect()
        };
        ...
        for mut c in candidates {
            let balance_slot = {
                let cached = self.state.read().await.balance_slot_cache.get(&c.token_a).copied();
                match cached {
                    Some(s) => s,
                    None => {
                        let s = find_balance_slot(provider, c.token_a).await;
                        self.state.write().await.balance_slot_cache.insert(c.token_a, s);
                        s
                    }
                }
            };
            ...
            let (passed, net_profit) = simulate(
                &c, &self.http_rpc, block_number,
                self.executor_addr, self.treasury, gas_price, balance_slot,
            ...
            if passed {
                ...
                if let Err(e) = submit_bundle(
                    &c, self.treasury, self.executor_addr, &self.wallet, &self.http_rpc,
                ).await {
...
async fn main() -> Result<()> {
    ...
    let executor_addr: H160 = std::env::var("EXECUTOR_ADDR")
        ...
        .parse().context("EXECUTOR_ADDR: invalid address")?;

    let treasury: H160 = std::env::var("TREASURY_ADDR")
        .context("TREASURY_ADDR not set")?
        .parse().context("TREASURY_ADDR: invalid address")?;
